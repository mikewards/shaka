...
object FishingIntelRoutes {
    ...
    fun getIntelForRegion(regionId: String, since: String, tzOffset: Int? = null): SpotIntelResponse? {
        ...
        // Insights are pre-generated by FishingIntelPrefetchJob; just read from DB.
        // If the job hasn't run yet for this slot, the user still gets species/trend data.
        val finalKeyInsights = FishingIntelDb.getRegionInsights(normalizedRegionId, slotKey) ?: emptyList()
    ...
    /**
     * Pre-generate AI insights for a region's current time slot.
     * Called by FishingIntelPrefetchJob after scraping so the request path never blocks on AI.
     */
    suspend fun prefetchRegionInsights(regionId: String, tzOffsetHours: Int = -8) {
        val normalizedRegionId = when (regionId.lowercase()) {
            "socal" -> "so_cal"
            else -> regionId.lowercase()
        }
        ...
        val existing = FishingIntelDb.getRegionInsights(normalizedRegionId, slotKey)
        if (!existing.isNullOrEmpty()) {
            logger.info("Region insights already cached for $normalizedRegionId slot=$slotKey")
            return
        }

        val allReports = FishingIntelDb.getReportsForRegion(normalizedRegionId, hoursBack = 168)
        if (allReports.isEmpty()) {
            logger.info("No reports for $normalizedRegionId, skipping insight generation")
            return
        }
        ...
        val speciesSummary = allSpeciesKeys.mapNotNull { species ->
            ...
            "${formatSpeciesName(species)}: $recentTotal (last 3 days), $baseTotal (prior 3 days), $trend ${percentChange}%"
        ...
        if (!insights.isNullOrEmpty()) {
            try {
                FishingIntelDb.setRegionInsights(normalizedRegionId, slotKey, insights)
                logger.info("Pre-generated ${insights.size} insights for $normalizedRegionId slot=$slotKey")
            } catch (e: Exception) {
                logger.warn("Failed to persist region insights: ${e.message}")
            }
        } else {
            logger.warn("AI insight generation returned empty for $normalizedRegionId slot=$slotKey")
        }
    }
