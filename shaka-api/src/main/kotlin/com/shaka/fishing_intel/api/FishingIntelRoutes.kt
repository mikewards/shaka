...
object FishingIntelRoutes {
    ...
    private val sourceNames = mapOf(
        "sportfishing-report" to "SportFishingReport.com"
    )

    private val regionLabels = mapOf(
        "wa_coast" to "WA Coast", "or_coast" to "OR Coast",
        "norcal" to "NorCal", "north_coast" to "North Coast",
        "bay_area" to "Bay Area", "central_coast" to "Central Coast",
        "ventura" to "Ventura", "la" to "Los Angeles",
        "orange" to "Orange County", "san_diego" to "San Diego",
        "baja" to "Baja"
    )
    ...
    fun getIntelForRegion(regionId: String, since: String, tzOffset: Int? = null): SpotIntelResponse? {
        ...
        val normalizedRegionId = regionId.lowercase()
        ...
        // Global insights (across all regions) are pre-generated by FishingIntelPrefetchJob.
        val finalKeyInsights = FishingIntelDb.getRegionInsights("all_regions", slotKey) ?: emptyList()
    ...
    /**
     * Pre-generate ONE global AI summary across ALL regions.
     * Called by FishingIntelPrefetchJob after scraping.
     */
    suspend fun prefetchGlobalInsights(tzOffsetHours: Int = -8) {
        ...
        if (allReports.isEmpty()) {
            logger.info("No reports across any region, skipping global insight generation")
            return
        }

        val dedupedReports = dedupeByLatest(allReports)
        val userZone = ZoneOffset.ofHours(tzOffsetHours)
        val nowInUserZone = ZonedDateTime.now(userZone)
        val startOfToday = nowInUserZone.toLocalDate().atStartOfDay(userZone).toInstant()
        val recentStart = startOfToday.minus(Duration.ofDays(3))
        val baselineStart = startOfToday.minus(Duration.ofDays(6))
        ...
        val speciesSummary = allSpeciesKeys.mapNotNull { species ->
            ...
            val topLanding = recent.topLanding ?: base.topLanding ?: ""
            "${formatSpeciesName(species)}: $recentTotal (last 3 days), $baseTotal (prior 3 days), $trend ${percentChange}%, top landing: $topLanding"
        }.take(15).joinToString("\n")

        val insights = FishingIntelAiService.generateRegionInsights(
            speciesSummary = speciesSummary,
            narrativeTldrs = emptyList(),
            totalReports = dedupedReports.size,
            regionLabel = "West Coast (WA to Baja)"
        )

        if (!insights.isNullOrEmpty()) {
            try {
                FishingIntelDb.setRegionInsights("all_regions", slotKey, insights)
                logger.info("Pre-generated ${insights.size} global insights for slot=$slotKey")
            } catch (e: Exception) {
                logger.warn("Failed to persist global insights: ${e.message}")
            }
        } else {
            logger.warn("AI global insight generation returned empty for slot=$slotKey")
        }
    }
