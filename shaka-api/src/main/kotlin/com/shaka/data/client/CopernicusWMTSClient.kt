...
/**
 * Client for Copernicus Marine WMTS service.
 * Provides REAL underwater visibility data (ZSD - Secchi disk depth) in meters.
 * 
 * Data source: Copernicus Global Ocean Colour, gap-free daily, 4km resolution
 * Dataset: cmems_obs-oc_glo_bgc-transp_nrt_l4-gapfree-multi-4km_P1D
 * 
 * This is NOT calculated from chlorophyll - it's actual measured water transparency!
 * Updated daily, gap-free (cloud gaps are interpolated).
 * 
...
class CopernicusWMTSClient {
    ...
    companion object {
        ...
        private const val ZSD_LAYER = "OCEANCOLOUR_GLO_BGC_L4_NRT_009_102/cmems_obs-oc_glo_bgc-transp_nrt_l4-gapfree-multi-4km_P1D_202311/ZSD"
     ...
     * Copernicus data typically has 1-2 day latency.
     */
    suspend fun getLatestVisibility(lat: Double, lon: Double): Double? {
        // Try last 3 days to find most recent data
    ...
    private fun parseZSDResponse(jsonResponse: String): Double? {
        return try {
            ...
            if (valueMatch != null && unitsMatch?.groupValues?.get(1) == "m") {
                val value = valueMatch.groupValues[1].toDoubleOrNull()
                // Sanity check: visibility should be between 1m and 60m
            ...
            logger.debug("ZSD parsing failed: ${e.message}")
            null
        }
    }
}
