...
| W8 | Wind snapshot ↔ live coherence (wind-audit Phase 4) | reuse T2's spot-detail snapshot + one `GET /spots/{spot}/wind/live` call. Probe-owned thresholds in `journeys.json`: snapshot vs live FROM-bearing divergence > **45°** (circular) → the derive tick / refetch cadence is serving users a different wind than reality; snapshot `windValidAt` age > **8 h** (hourly derive + 6 h refetch means it should never happen) → snapshot-age alert; `0 kts @ 0°` → fabricated-zero signature (Jun 2026 outage shape). Skips cleanly when the numeric wind fields are absent (pre-contract payload / local seed) |
